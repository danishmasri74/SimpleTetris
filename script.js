var Page = {
    ...
    Initialize: function () {
        ...
        if (this.IsSetup === false) {
            ...
            this.body.style.backgroundColor = 'white'; // Set the background color to white
            this.cvs.style.position = 'absolute';
            this.cvs.style.borderRadius = '10px'; // Set the border radius to create a curved border
            this.ctx = this.cvs.getContext('2d');
            // Add shadow to the canvas
            this.cvs.style.boxShadow = '0px 0px 10px rgba(0, 0, 0, 0.5)';
            this.IsSetup = true;
        }
        this.WindowChanged();
...
document.addEventListener("DOMContentLoaded", function () {
    // Set the title
    document.title = "Tetris Game";
    // Create the social media icons
    const socialIcons = document.createElement("div");
    socialIcons.className = "social-icons";
    const twitterLink = document.createElement("a");
    twitterLink.className = "black-icon";
    twitterLink.href = "https://twitter.com/StylizedWolf";
    twitterLink.target = "_blank";
    twitterLink.innerHTML = '<i class="fab fa-twitter"></i>';
    const githubLink = document.createElement("a");
    githubLink.className = "black-icon";
    githubLink.href = "https://github.com/danishmasri74";
    githubLink.target = "_blank";
    githubLink.innerHTML = '<i class="fab fa-github"></i>';
    const instagramLink = document.createElement("a");
    instagramLink.className = "black-icon";
    instagramLink.href = "https://www.instagram.com/lunarlure_wolf_74";
    instagramLink.target = "_blank";
    instagramLink.innerHTML = '<i class="fab fa-instagram"></i>';
    const websiteLink = document.createElement("a");
    websiteLink.className = "black-icon";
    websiteLink.href = "https://my-projects-site.netlify.app/";
    websiteLink.target = "_blank";
    websiteLink.innerHTML = '<i class="fas fa-globe"></i>';
    // Append the social media icons to the socialIcons div
    socialIcons.appendChild(twitterLink);
    socialIcons.appendChild(githubLink);
    socialIcons.appendChild(instagramLink);
    socialIcons.appendChild(websiteLink);
    // Append the socialIcons div to the document
    document.querySelector(".tetris-panel").appendChild(socialIcons);
});
...
Page.Game = new DrawAreaObj(0, 0, 10, 20, function () {
    var uDrawSize = Page.unitSize - 2,
        ...
        drawT,
        gameOverAnimation = 0;
    Page.ctx.fillStyle = 'rgb(255, 255, 255)';
    Page.ctx.fillRect(this.left, this.top, this.W, this.H);
    Page.ctx.strokeStyle = 'rgb(200, 200, 200)';
    Page.ctx.lineWidth = 0.5;
    for (var x = 0; x <= this.W; x += Page.unitSize) {
        Page.ctx.beginPath();
        Page.ctx.moveTo(this.left + x, this.top);
        Page.ctx.lineTo(this.left + x, this.top + this.H);
        Page.ctx.stroke();
    }
    for (var y = 0; y <= this.H; y += Page.unitSize) {
        Page.ctx.beginPath();
        Page.ctx.moveTo(this.left, this.top + y);
        Page.ctx.lineTo(this.left + this.W, this.top + y);
        Page.ctx.stroke();
    }
    for (var i = 0; i < GM.StaticUnits.length; i++) {
        for (var j = 0; j < GM.StaticUnits[i].length; j++) {
            ...
            if (uValue !== 0) {
                ...
                Page.ctx.fillStyle = 'rgb(200, 200, 200)';
    ...
    if (!GM.IsAlive) {
        ...
        var yOffset = Math.sin(gameOverAnimation) * 10;
        var textY = this.H / 8 + yOffset;
        DrawText("GAME OVER", 'rgb(0, 0, 0)', '50', 'center', this.W / 2, textY);
    }
    function DrawText(text, color, size, align, x, y) {
        Page.ctx.fillStyle = color;
        Page.ctx.font = "bold " + size + "px Arial"; // Add "bold" to make the font thicker
        Page.ctx.textAlign = align;
        Page.ctx.globalCompositeOperation = 'source-over';
        Page.ctx.fillText(text, x, y);
    }
});

Page.UpcomingA = new DrawAreaObj(11.2, 5, 2.8, 2.8, function () {
    ...
    Page.ctx.save();
    // Set the background color and shadows
    Page.ctx.fillStyle = 'rgb(50, 50, 50)';
    Page.ctx.shadowColor = 'rgba(0, 0, 0, 0.3)';
    Page.ctx.shadowBlur = 5;
    Page.ctx.shadowOffsetX = 2;
    Page.ctx.shadowOffsetY = 2;
    // Define the radius of the curves
    const cornerRadius = 10;
    // Draw the shadowed background with curves
    Page.ctx.beginPath();
    Page.ctx.moveTo(this.left + cornerRadius, this.top);
    Page.ctx.lineTo(this.left + this.W - cornerRadius, this.top);
    Page.ctx.arcTo(this.left + this.W, this.top, this.left + this.W, this.top + cornerRadius, cornerRadius);
    Page.ctx.lineTo(this.left + this.W, this.top + this.H - cornerRadius);
    Page.ctx.arcTo(this.left + this.W, this.top + this.H, this.left + this.W - cornerRadius, this.top + this.H, cornerRadius);
    Page.ctx.lineTo(this.left + cornerRadius, this.top + this.H);
    Page.ctx.arcTo(this.left, this.top + this.H, this.left, this.top + this.H - cornerRadius, cornerRadius);
    Page.ctx.lineTo(this.left, this.top + cornerRadius);
    Page.ctx.arcTo(this.left, this.top, this.left + cornerRadius, this.top, cornerRadius);
    Page.ctx.closePath();
    Page.ctx.fill();
...
Page.UpcomingB = new DrawAreaObj(11.2, 9, 2.8, 2.8, function () {
...
Page.UpcomingC = new DrawAreaObj(11.2, 13, 2.8, 2.8, function () {
...
Page.ScoreBarHigh = new DrawAreaObj(10.5, 0.5, 4.5, 1, function () {
    // draw the score area back bar
    Page.ctx.fillStyle = 'rgb(224, 56, 56)';
    Page.ctx.fillRect(this.left, this.top, this.W, this.H);
    // Draw the trophy symbol
    var miniUnit = Page.unitSize * 0.01;
    // Load the image
    var trophyImage = new Image();
    trophyImage.onload = function () {
        Page.ctx.drawImage(
            trophyImage,
            this.left + miniUnit * 33,
            this.top + this.H - miniUnit * 68 - miniUnit * 15, // Adjust the top position by 10 units
            miniUnit * 48 * 1.5,
            miniUnit * 48 * 1.5
        );
    }.bind(this);
    ...
    var text = ("00000000" + GM.ScoreHigh).slice(-7);
    var left = this.left + this.W - 4;
    var top = this.top + Page.unitSize * 0.8;
    var size = Math.floor(Page.unitSize * 0.8) + 0.5;
    DrawText(text, 'rgb(255,232,96)', '500', 'right', size, left, top);
});

Page.ScoreBarCur = new DrawAreaObj(10.5, 1.7, 4.5, 1, function () {

    // draw the score area back bar
    Page.ctx.fillStyle = 'rgb(185, 56, 224)';
    Page.ctx.fillRect(this.left, this.top, this.W, this.H);
    ...
    DrawText(text, 'rgb(33,33,33)', '900', 'center', size, left, top);
